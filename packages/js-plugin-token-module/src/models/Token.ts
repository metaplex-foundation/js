...
import { amount, Pda, SplTokenAmount, token } from '@metaplex-foundation/js';
import { assert, Option } from '@metaplex-foundation/js';
...
export const toTokenWithMint = (
  ...
  const transformedToken = toToken(tokenAccount);
  return {
    ...transformedToken,
    model: 'tokenWithMint',
    mint: mintModel,
    amount: amount(transformedToken.amount.basisPoints, mintModel.currency),
    delegateAmount: amount(
      transformedToken.delegateAmount.basisPoints,
      mintModel.currency
    ),
  };
};
