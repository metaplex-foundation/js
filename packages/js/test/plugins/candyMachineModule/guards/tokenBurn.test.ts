...
test('[candyMachineModule] tokenBurn guard: it may explicitely provide a burn authority', async (t) => {
  // Given a payer with one token.
  const mx = await metaplex();
  const payer = await createWallet(mx, 10);
  const { token: payerTokens } = await mx
  ...
  // And a delegate authority on that token.
  const delegateAuthority = Keypair.generate();
  await mx
    .tokens()
    .approveDelegateAuthority({
      owner: payer,
      mintAddress: payerTokens.mint.address,
  ...
  // When the payer mints from it by explicitly providing its delegate authority.
  const { nft } = await mx
    .candyMachines()
    .mint({
      candyMachine,
      ...
      guards: {
        tokenBurn: {
          tokenBurnAuthority: delegateAuthority,
        },
      },
    })
    .run();
