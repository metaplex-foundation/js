...
test('[nftModule] the owner of an NFT can burn it', async (t: Test) => {
  // Given an existing NFT.
  const mx = await metaplex();
  const owner = Keypair.generate();
  const nft = await createNft(mx, { tokenOwner: owner.publicKey });

  // When the owner burns the NFT.
  await mx.nfts().burn({ mintAddress: nft.address, owner });
...
test('[nftModule] it decreases the collection size when deleting the NFT', async (t: Test) => {
  ...
  // When the owner burns the NFT and provides the collection address.
  await mx.nfts().burn({
...
test('[nftModule] the update authority of an NFT cannot burn it', async (t: Test) => {
  // Given an existing NFT.
  const mx = await metaplex();
  const owner = Keypair.generate();
  ...
  // When the update authority tries to burn the NFT.
  const promise = mx.nfts().burn({
    mintAddress: nft.address,
